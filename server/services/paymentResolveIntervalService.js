...
const { getAllNotResolvedPayments, retrievePaymentById, getOpenCheckoutSessions, retrieveCheckout } = require("./payments");
const { PaymentsEventEmitter } = require('../events/emitters/PaymentEmitter');

(async () => {
  setInterval(async () => {
    const notResolvedPaymentes = await db.payment.findMany({ where: { status: "PENDING" } });
    ...
    for (const dbPayment of notResolvedPaymentes) {
      ...
      if (stripePayment.payment_status === "paid") {
        const payment = await db.payment.update({
          where: { stripeId: dbPayment.stripeId }, data: {
            resolved: true,
        ...
        PaymentsEventEmitter.getInstance().emit(process.env.EVENT_PAYMENT_SUCCESS, payment);
